...
//            long creatTime = string2Date(query.getString(query.getColumnIndex(DBConstant.createTime)));
    ...
    public List<GpBean> selectZiXuanTodayDetail(String dataCode) {
        String orderBy = DBConstant.create_time + " asc";
        String year= CalendarUtil.getYear()+"";
        String month= CalendarUtil.getMonth()+"";
        String day= CalendarUtil.getDay()+"";
        String sql = DBConstant.code + " =? and "+DBConstant.gp_year+"=? and "+DBConstant.gp_month+"=? and "+DBConstant.gp_day+"=? ";
        SQLiteDatabase db = getWritableDatabase();
        Cursor query = db.query(DBManager.T_EveryMinute,
                new String[]{
                        DBConstant._id,
                        DBConstant.uid,
                        DBConstant.gp_uid,
                        ...
                        DBConstant.shi_ying_lv_dong,
                        DBConstant.shi_ying_lv_jing
                },  sql , new String[]{dataCode,year,month,day}, null, null, orderBy);
        List<GpBean> list = new ArrayList<GpBean>();
        GpBean bean;
        while (query.moveToNext()) {
            bean = new GpBean();
